...
# Pooled statistics across non-Syria topics

Calculate overall Mean and SD per arm across topics (Drones, Iran, Wall) using weighted pooling.

```{r pooled-non-syria-stats}
# Read the data
non_syria_data <- read.csv(file.path(data_dir, "Non-Syria_topic_stats.csv"))

# Calculate SD from SE and pooled statistics per arm across topics
pooled_stats <- non_syria_data %>%
  # Calculate SD from SE
  mutate(SD = SE * sqrt(n)) %>%
  # Group by Metric and Arm to pool across topics
  group_by(Metric, Arm) %>%
  summarise(
    # Weighted mean (weighted by sample size)
    Overall_Mean = sum(Mean * n) / sum(n),
    
    # Pooled SD accounting for both 
    # within-group variance and between-group variance
    Overall_SD = sqrt(
      sum((n - 1) * SD^2 + n * (Mean - sum(Mean * n) / sum(n))^2) / 
      (sum(n) - 1)
    ),
    ...
    # Number of topics pooled
    n_topics = n(),
    
    .groups = "drop"
  ) %>%
  # Format for better display
  mutate(across(c(Overall_Mean, Overall_SD), ~round(., 2))) %>%
  arrange(Metric, Arm)
...
write.csv(pooled_stats, file.path(output_dir, "pooled_non_syria_stats.csv"), row.names = FALSE)
```
...
compute_cohens_d <- function(df) {
  ...
  if (nrow(res2) > 0) {
    ...
    if (all(c("M_treatment", "M_control") %in% names(res2))) {
      ...
      dir[is.na(dir)] <- 1   # falls ein NA im Unterschied
      res2$yi <- res2$yi * dir
    }
...
# Determine stability
stability_assessment <- if(effect_range < 0.1) {
  "STABLE (range < 0.10)"
} else if(effect_range < 0.2) {
  "MODERATE stability (range < 0.20)"
} else {
  "HIGH variability (range ≥ 0.20)"
}
